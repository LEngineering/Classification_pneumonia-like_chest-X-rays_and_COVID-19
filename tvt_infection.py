...
datasetPath='/Users/*/dataset' # add your path

def image_class_extract(fimagePath):
    
    fimgPath = list(paths.list_images(fimagePath)) 
    ...
    for img in fimgPath:
	    ...
	    fImdata.append(image)
	    fImlabels.append(clabel)

    # Conversion to np.array of Data + pixel range ([0,255]) and Labels 
    fImdata = np.array(fImdata) / 255.0
    fImlabels = np.array(fImlabels)

    # one-hot encoding on the labels using LabelBinarizer()
    flb = LabelBinarizer()
    fImlabelst = flb.fit_transform(fImlabels)
    fImlabelsc = to_categorical(fImlabelst) 
    
     
    return (fImlabelsc,fImdata,flb)


def test_pred(ftest_X, ftest_Y,fbatch_size,fmodel,flb):
    
# make predictions on the testing set
    print("----------Evaluating test set----------")
    testpred = fmodel.predict(ftest_X, fbatch_size)
...
#Classification report #########################
    print(classification_report(ftest_Y.argmax(axis=1),
    testpred_a,target_names=flb.classes_))

# Compute ROC curve and ROC area for each class
    FPr = dict()
    TPr = dict()
    roc_auc = dict()
    for i in range(ftest_Y.shape[-1]):
        FPr[i], TPr[i], _ = roc_curve(ftest_Y[:, i], testpred[:, i])
        roc_auc[i] = auc(FPr[i], TPr[i])

# Compute micro-average ROC curve and ROC area
    FPr["micro"], TPr["micro"], _ = roc_curve(ftest_Y.ravel(), testpred.ravel())
...
#sensitivity,specificity and accuracy
    cf_m = confusion_matrix(ftest_Y.argmax(axis=1), testpred_a)
...
Imlabels,Imdata,lb=image_class_extract(datasetPath)
...
#split again train into validation and train (0.25*0.8=0.2 for validation)
...
test_pred(test_X,test_Y,BS,model,lb)
...
#New dataset for testing based on our model already trained =============================================
    

imagePath= '/Users/crist/Documents/PYTHON/ProjectsDL/COVID/Pneumonia/covid'

Imlabels,Imdata,lb=image_class_extract(imagePath) 
test_pred(Imdata,Imlabels,BS,model,lb) 
